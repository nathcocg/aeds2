...
void insertion(struct players *p, int entry_position){
    int k = 10;
    for(int i = 1; i< entry_position; i++){
        struct players tmp = p[i];
        int j = (i<k) ? (i-1):(k-1);

        while(((j>=0) && (p[j].birth > tmp.birth)) || (((j>=0) && (p[j].birth==tmp.birth)) && (strcmp(p[j].name, tmp.name)>0))){
            p[j+1] = p[j];
            j--;
        }
        p[j+1] = tmp;
...
int main(int argc, char *argv[]){
    ...
    struct players insertion_players[playersMAX];
    ...
    while(!isEnd(entry)){
        ...
        if(entry_position < playersMAX){
            insertion_players[entry_position] = clone(players[j]);
            entry_position++;
            scanf(" %[^\n]",entry);
        }
    }
    insertion(insertion_players, entry_position);
    for(int k = 0; k<10; k++){
        print(insertion_players, k);
    }
