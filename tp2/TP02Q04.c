...
void selection(struct players *p, int entry_position){
    for(int i = 0; i < entry_position - 1; i++){
        int smallest = i;
        for(int j = i+1; j<entry_position; j++){
            if(strcmp(p[j].name, p[smallest].name)<0){
                smallest = j;
            }
        }
        swap(&p[i], &p[smallest]);
...
bool binary_search(struct players *p, char entry[MAX], int entry_position){
    bool result = 0;
    int r = (entry_position -1);
    int l = 0;
    int middle;

    while(l<=r){
        middle = (l+r)/2;
        if(strcmp(entry, p[middle].name) == 0){
            result = 1;
            l = r+1;
            break;
        } else if(strcmp(entry, p[middle].name)>0){
            l = middle+1;
        }else{
            r = middle - 1;
        }
...
int main(int argc, char *argv[]){
    ...
    struct players selection_players[playersMAX];
    ...
    while(!isEnd(entry)){
        ...
        if(entry_position < playersMAX){
            selection_players[entry_position] = clone(players[j]);
            entry_position++;
            scanf(" %[^\n]",entry);
        }
    }
    selection(selection_players, entry_position);
    /*for(int k = 0; k<entry_position; k++){
        print(selection_players, k);
    }*/

    /*------PESQUISA BINÁRIA-------*/ //-> fazer com outro algoritmo de ordenação talvez
   char name_entry[MAX];
    scanf(" %[^\n]",name_entry);
    //printf("%s\n", name_entry);
    int r = 0;
    //printf("%d\n", r);
    while(!isEnd(name_entry)){
        r = binary_search(selection_players, name_entry, entry_position);
        //printf("%d\n", r);
        if(r == 1){
            printf("SIM\n");
        } else {
        ...
        r = 0;
        //printf("So i work real hard each and every day\n");
        scanf(" %[^\n]",name_entry);
    }
    return 0;
}    