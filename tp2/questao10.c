...
void quicksortRec(struct players *p, int l, int r){
    int i = l, j = r;
    struct players pivo = p[((r-1)+l)/2];
    while(i<=j){
        while((strcmp(p[i].state, pivo.state)<0) || ((strcmp(p[i].state, pivo.state)==0) && (strcmp(p[i].name, pivo.name)<0))){
            i++;
        }
        while((strcmp(p[j].state, pivo.state)>0) || ((strcmp(p[j].state, pivo.state)==0) && (strcmp(p[j].name, pivo.name)>0))){
            j--;
        }
        if(i<=j){
            swap(p+i, p+j);
...
void quicksort(struct players *p, int entry_position){
    quicksortRec(p, 0, entry_position-1);
}

int main(int argc, char *argv[]){
    struct players players[playersMAX];//₢ria um vetor para TODOS os jogadores do arquivo
    struct players quick_players[playersMAX];
    ...
    while(!isEnd(entry)){
        ...
        if(entry_position < playersMAX){
            quick_players[entry_position] = clone(players[j]);
            entry_position++;
            scanf(" %[^\n]",entry);
        }
    }
    quicksort(quick_players,entry_position);
    for(int k = 0; k<entry_position; k++){
        print(quick_players, k);
    }
