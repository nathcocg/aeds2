...
public class TP02Q05 {
    ...
    public static void main(String[] args) { 
        ...
        Players[] selection = new Players[MAX];
        ...
        try {
            ...
            while(!(entry != null && entry.equals("FIM"))){
                ...
                if(entry_position<selection.length){
                    selection[entry_position] = new Players();
                    selection[entry_position].setId(players[j].getId());
                    selection[entry_position].setName(players[j].getName());
                    selection[entry_position].setHeight(players[j].getHeight());
                    selection[entry_position].setWeight(players[j].getWeight());
                    selection[entry_position].setCollege(players[j].getCollege());
                    selection[entry_position].setBirth(players[j].getBirth());
                    selection[entry_position].setCity(players[j].getCity());
                    selection[entry_position].setState(players[j].getState());
                    entry_position++;
                }
                entry = MyIO.readLine();
            }
            ...
            */
            ...
            int e, s;
            for(s = 0; s<entry_position-1; s++){
                int smallerIndex = s;
                String smaller = selection[s].getName(); //o menor é o elemento da iteração externa
                for(e = (s+1); e<entry_position; e++){
                    if(smaller.compareTo(selection[e].getName())>0){
                        smaller = selection[e].getName();
                        smallerIndex = e;
                    }
                }
                //swap(menor, i);
                temp = selection[s];
                selection[s] = selection[smallerIndex];
                selection[smallerIndex] = temp;
