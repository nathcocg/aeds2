...
void quicksort(struct players *p, int l, int r){//todos os não informados vão para o fim da ordenação, o que é a saída esperada
    int i = l, j = r;
    struct players pivo = p[(r+l)/2];
    while(i<=j){
        while((strcmp(p[i].state, pivo.state)<0) || ((strcmp(p[i].state, pivo.state)==0) && (strcmp(p[i].name, pivo.name)<0))){
            i++;
        }
        while((strcmp(p[j].state, pivo.state)>0)||((strcmp(p[j].state, pivo.state)==0) && (strcmp(p[i].name, pivo.name)>0))){
            j--;
        }
        if(i<=j){
            swap(p+i, p+j);
...
int main(int argc, char *argv[]){
    ...
    quicksort(quick_players, 0, entry_position-1);
