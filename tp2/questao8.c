...
void placing_players(struct players p[]){ //le o arquivo, cria um jogador para cada linha e atribui suas características
    FILE *arq;
    arq = fopen("players.csv", "r");//trocar antes de mandar no verde
    char line[MAX];
...
void color_insertion(struct players *p, int entry_position, int color, int h){
    for(int i = (h + color); i<entry_position; i+=h){
        struct players tmp = p[i];
        int j = i - h;
        while(((j>=0) && (p[j].weight > tmp.weight)) || (((j>=0) && (p[j].weight==tmp.weight) && strcmp(p[j].name, tmp.name)>0))){
            p[j+h] = p[j];
            j-=h;
        }
        p[j+h] = tmp;
...
void shellsort(struct players *p, int entry_position){
    int h = 1;
    do{
        h = (h*3)+1;
    }while(h<entry_position);
    do{
        h/=3;
        for(int color = 0; color<h; color++){
            color_insertion(p, entry_position, color, h);
        }
    }while(h!=1);
}
...
int main(int argc, char *argv[]){
    ...
    struct players shell_players[playersMAX];
    ...
    while(!isEnd(entry)){
        ...
        if(entry_position < playersMAX){
            shell_players[entry_position] = clone(players[j]);
            entry_position++;
            scanf(" %[^\n]",entry);
        }
    }
    shellsort(shell_players, entry_position);
    for(int k = 0; k<entry_position; k++){
        print(shell_players, k);
    }
