...
void selection(struct players *p, int entry_position, int i){
    if(i>=entry_position - 1){
        return;//para as chamadas recursivas quando chega na condição de parada
    }
    int smallest = i;
    ...
    swap(&p[i], &p[smallest]);
    if(i+1< entry_position){
        selection(p, entry_position, i+1);
    }
}
...
int main(int argc, char *argv[]){
    ...
    selection(selection_players, entry_position, 0);
