...
public class TP02Q09 {
    ...
    /*funções do heapsort */
    public static Players biggestSon(int i, int heap_size, Players[] p){
        Players son = new Players();
        if((2*i == heap_size) || (p[2*i].getHeight() > p[2*i+1].getHeight()) || ((p[2*i].getHeight() == p[2*i+1].getHeight()) && (p[2*i].getName().compareTo(p[2*i+1].getName())>0))){
            son = p[2*i];
        } else{
            son = p[2*i+1];
        }
    ...
    public static void construct(int heap_size, Players[] p){
        /*for(int i = heap_size; i>1 && ((p[i].getHeight() > p[i/2].getHeight()) || ((p[i].getHeight() == p[i/2].getHeight()) && (p[i].getName().compareTo(p[i/2].getName())>0))); i/=2){
            Players tmp = p[i];
            p[i] = p[i/2];
            p[i/2] = tmp;
        }*/
    ...
    public static void reconstruct(int heap_size, Players[] p){
        int i = 1;
       while(i<= (heap_size/2)){
            Players son = biggestSon(i, heap_size, p);
             /*if(p[i].getHeight() < son.getHeight() || ((p[i].getHeight() == son.getHeight()) && (p[i].getName().compareTo(son.getName())<0))){
                Players tmp = p[i];
                p[i] = son;
                son = tmp;
            }*/
            int index = 2 * i;
            if (index < heap_size && (p[index].getHeight() < p[index + 1].getHeight() || (p[index].getHeight() == p[index + 1].getHeight() && p[index].getName().compareTo(p[index + 1].getName()) < 0))) {
                index++;
            }
