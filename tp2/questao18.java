...
public class questao18 {
    ...
    public static void quicksort(int esq, int dir, Players[] array) {
        int i = esq, j = dir;
        Players pivo = new Players();
        pivo = array[(dir+esq)/2];
        while (i <= j) {
            while (array[i].getState().compareTo(pivo.getState())<0 || ((array[i].getState().compareTo(pivo.getState())==0) && (array[i].getName().compareTo(pivo.getName())<0))) {i++;}
            while (array[j].getState().compareTo(pivo.getState())>0 || ((array[j].getState().compareTo(pivo.getState())==0) && (array[j].getName().compareTo(pivo.getName())>0))) {j--;}
            if (i <= j) {
                //swap(i, j);
    ...
    public static void main(String[] args) { 
        ...
        try {
            ...
            for(int b = 0; b<entry_position; b++){
                if(quick_players[b].getState().equals("")){
                    quick_players[b].setState("zzzzz");
                }
            }

            quicksort(0, entry_position-1, quick_players);
            for(int k = 0; k<a; k++){
