...
struct players getMax(struct players *p, int n){
    struct players biggest = p[0];
    for(int i = 1; i<n; i++){
        if(biggest.id < p[i].id){
            biggest = p[i];
        }
    }
    return biggest;
...
void radCountingSort(struct players *p, int n, int exp){
    int count[10];
    struct players output[playersMAX];
    ...
    for (int i = n-1; i >= 0; i--) {
        output[count[(p[i].id/exp) % 10] - 1] = p[i];
        count[(p[i].id/exp) % 10]--;
    }
    for (int i = 0; i < n; i++) {
        p[i] = output[i];
...
void radixsort(struct players *p, int n){
    struct players b = getMax(p, n);
    for(int exp = 1; b.id/exp>0; exp*=10){
        radCountingSort(p, n, exp);
    }
}
...
int main(int argc, char *argv[]){
    ...
    struct players radix_players[playersMAX];
    ...
    while(!isEnd(entry)){
        ...
        if(entry_position < playersMAX){
            radix_players[entry_position] = clone(players[j]);
            entry_position++;
            scanf(" %[^\n]",entry);
        }
    }
    
     radixsort(radix_players, entry_position);

    for(int k = 0; k<entry_position; k++){
        print(radix_players, k);
    }
